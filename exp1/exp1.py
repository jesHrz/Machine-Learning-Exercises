...
def plot_cost(X, y):
    ...
    for i in range(100):
        for j in range(100):
            cost_value[i][j] = Linear_Regression.cost_value(X, y, np.array([theta_0[i], theta_1[j]]).reshape(-1, 1))
    theta_0, theta_1 = np.meshgrid(theta_0, theta_1)
    # 三维图
    Axes3D(figure).plot_surface(theta_0, theta_1, cost_value)
    # 等高线
    # plt.contourf(theta_0, theta_1, cost_value, 40, alpha=0.8)
    # cb = plt.colorbar()
    # cb.set_label('cost')
    plt.xlabel('theta_0')
    plt.ylabel('theta_1')
